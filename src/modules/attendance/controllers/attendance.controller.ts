...
export class AttendanceController {
    ...
    @ApiOperation({
        ...
        description: 'Retrieve attendance summary for team members (managers only)',
    })
    @ApiQuery({ name: 'startDate', type: String, example: '2025-10-01' })
    @ApiQuery({ name: 'endDate', type: String, example: '2025-10-31' })
    @ApiResponse({
    ...
    async getTeamAttendance(
        ...
        const startDate = new Date(startDateStr);
        const endDate = new Date(endDateStr);
        return await this.attendanceService.getTeamAttendance(req.user.userId, startDate, endDate);
    }
    ...
    @Get('team/:employeeId')
    @ApiOperation({
        summary: 'Get individual team member attendance',
        description: 'Retrieve detailed attendance for a specific team member (managers only)',
    })
    @ApiQuery({ name: 'startDate', type: String, example: '2025-10-01' })
    @ApiQuery({ name: 'endDate', type: String, example: '2025-10-31' })
    @ApiResponse({
        status: 200,
        description: 'Individual team member attendance',
        schema: {
            example: [
                {
                    id: 'attendance-uuid-1',
    ...
    async getTeamMemberAttendance(
        ...
        const startDate = new Date(startDateStr);
        const endDate = new Date(endDateStr);
        return await this.attendanceService.getTeamMemberAttendance(
            req.user.userId,
            employeeId,
            startDate,
            endDate,
